...
pub const IO = struct {
    ...
    /// State machine for the cancel_all call. The cancelation of
    /// all requests (both in fligh and already submitted) should
    /// wait until ALL requests are cancelled. We do this either in
    /// batch or one-by-one.
    /// The state machine goes:
    ///  "inactive" -> (next -------> queued) -> "done"
    ///                   ^--- wait <---
    /// A series of ASYNC_CANCEL requests is submitted
    /// to the kernel, with help of io_uring_prep_cancel
    /// and corresponding requests are yanked from processing.
    ///
    /// The above cancellation targets all SQEs and once the awaiting
    /// list is empty we're done and hence end up in the "done" state.
    ...
    /// This struct holds the data needed for a single epoll operation.
    pub const Completion = struct {
        ...
        fn prep(completion: *Completion, epoll_fd: fd_t) !void {
            ...
            _ = result catch |err| switch (err) {
                    ...
                    (completion.callback)(
                        @ptrCast(@alignCast(completion.contex)),
                        completion,
                        err,
                    );
                },
